 ...
 *
 */
package net.sourceforge.fake.net;

import net.sourceforge.jns.dynamic.DynamicScheduler;
...
/**
 * This is the "fake" socket which can be substituted a
 * java.net.MulticastSocket. Instead of sending and receiving messages onto a
 * real network, this fake socket will connect to and schedule sends and call
 * receives on the JNS 1.7 network simulator.
 *
 * In order to use the fake.net.MulticastSocket in your code, replace all
 * occurences of "import java.net.MulticastSocket" with
 * "fake.net.MulticastSocket".
 *
 * And change all calls to the MulticastSocket contructor to be
 * MulticastSocket(port, InternetAddress) where InternetAddress is the host
 * which the socket should pretend to be. (port is some bogus value).
 *
 * Then you need to start the DynamicScheduler with the correct network
 * simulation information. (See the documentation that comes with JNS-1.7 on how
 * to do this).
 *
 * Finally, change fake.net.Preferences, to allow SERVER_HOST_NAME and
 * SERVER_PORT_NO to point to where the dynamic scheduler is running.
 *
 * After this, your code should run as normal (unless you're using some bits of
 * the MulticastSocket API that the fake version doesn't support).
 *
 * NOTE: If you use bits of the MulticastSocket API that I haven't yet
 * implemented, then calling the method will cause an error message and
 * System.exit(-1). Annoying, yes, but I don't have the time to emulate all the
 * behaviour of a MulticastSocket, so I am implementing only the bits I need.
...
public class MulticastSocket extends java.net.MulticastSocket {
    ...
    /**
     * In fake.net.MulticastSocket, this schedules a send event with the JNS
     * simulator
     *
     * Note: This will currently also allow unicasts, just set the address in
     * the DatagramPacket to be a unicast address instead.
     *
     * @throws java.io.IOException
     */
    @Override
    ...
    /**
     * In fake.net.MulticastSocket. This function calls the receive function on
