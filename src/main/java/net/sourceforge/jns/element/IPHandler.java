 ...
 *
 * - Process time to live field - look at copy_options field in ip packets -
 */
package net.sourceforge.jns.element;

import net.sourceforge.jns.Simulator;
import net.sourceforge.jns.agent.Agent;
import net.sourceforge.jns.agent.CL_Agent;
import net.sourceforge.jns.command.Command;
import net.sourceforge.jns.command.ElementUpdateCommand;
import net.sourceforge.jns.util.IPAddr;
import net.sourceforge.jns.util.Preferences;
import net.sourceforge.jns.util.RoutingTable;
...
public class IPHandler extends Element implements CL_Agent {
    ...
    private net.sourceforge.jns.util.Queue m_packets_send;  // Packets waiting to be sent
    private net.sourceforge.jns.util.Queue m_packets_recv;  // Received packets to be processed
    ...
    public IPHandler() {
        ...
        m_packets_send = new net.sourceforge.jns.util.Queue();
        m_packets_recv = new net.sourceforge.jns.util.Queue();
    ...
    public void update() {
        ...
        while (m_packets_send.size() > 0) {
            ...
            for (int j = 0; j < targets.size(); j++) {
                ...
                Interface interfaceTarget = ((Interface) targets.get(j));
                System.out.println("Packet dest: " + curpacket.destination);
                System.out.println("Packet length: " + curpacket.length);
                boolean tempCanSend = interfaceTarget.canSend(curpacket.destination, curpacket.length);
                if (!tempCanSend) {
    ...
    /**
     * Private function that processes a packet to be sent to a certain
     * destination
     */
    /**
...
class HigherAgent {
    ...
    public net.sourceforge.jns.util.Queue queue;
    public Agent agent;

    public HigherAgent(Agent agent) {
        this.agent = agent;
        queue = new net.sourceforge.jns.util.Queue();
    }
}
