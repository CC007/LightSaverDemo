package net.sourceforge.jns.dynamic;

/**
 * dynamic.DynamicSchedulerImpl.java
 *
 * This class allows for simple creation of a network of interconnected JNS
 * nodes with duplex links between them. It also allows for scheduling events
 * dynamically with the JNS simulator, through RMI calls. The main purpose of
 * this class is to be an RMI gateway for the fake.net.MulticastSocket classes
 * used to test java implementations of network protocols.
 *
 * @author Einar Vollset <einar.vollset@ncl.ac.uk>
 *
 */
import net.sourceforge.jns.element.Link;
import net.sourceforge.jns.element.Interface;
import net.sourceforge.jns.element.Node;
import net.sourceforge.jns.element.DuplexInterface;
import net.sourceforge.jns.element.IPHandler;
import net.sourceforge.jns.element.IPPacket;
import net.sourceforge.jns.element.DuplexLink;
import net.sourceforge.jns.Simulator;
import net.sourceforge.jns.agent.Agent;
import net.sourceforge.jns.command.StopCommand;
import net.sourceforge.jns.trace.JavisTrace;
import net.sourceforge.jns.trace.Trace;
import net.sourceforge.jns.util.IPAddr;
import net.sourceforge.jns.util.Protocols;
import net.sourceforge.jns.util.Queue;
...
public class DynamicSchedulerImpl extends UnicastRemoteObject implements DynamicScheduler, Agent {
    ...
    /**
     * This is a reference to the JNS simulator instance. This needs to get
     * passed to the
     */
    ...
    /**
     * This determines if the simulation has started, in which no more nodes can
     * be added to the simulation run
     */
     ...
     *
     * @param nameOfRun The name of this simulation run. Gives name to the JVS
     * file.
     * @param linkBandwidth The bandwidth of all the links between the nodes
     * (could change this to set it for each link added)
     * @param linkDelay The link delay of all the links between the nodes (could
     * change this to set it for each link added)
     * @param linkErrorRate The link error rate of all the links between the
     * nodes (could change this to set it for each link added)
     * @param subnetMask The subnet mask of all the routes given to the nodes
     * (could change this to set it for each link added)
     */
     ...
     * there is no need to add a link "the other way" This method can currently
    ...
    /**
     * This method schedules a sending of a given packet of data (an object)
     * from one node to the other. The sending is scheduled to happen at
    ...
    /**
     * This method will stop the simulator. TODO: Change this so the simulator
     * does not stop until all the participants have called stop()
     */
    @Override
    ...
    /**
     * This method will start the simulator. This is called by the main() method
     * in this class.
     */
     ...
     * clients, trying to attach a higher level agent does not make any sense.
     ...
     * agent, the lower level agent will attach itself to the higher-level agent
     ...
     * IPHandler when the the dynamic scheduler gets attached to the IPHandler.
     * This is so that packets can be passed up to the dynamic scheduler
     *
    ...
    /**
     * Indicate to this agent that a packet is waiting for collection. This is
     * used by lower-level agents to indicate to higher-level agents. Note that
     ...
     * <li>The function returns 'false' - In this case you are guaranteed to get
     * a READY_TO_SEND indication by the lower level agent you are using. Do not
     * try to send any packets but wait for a call to your indicate function
     * with that flag set. When you do get that call, you <b>must</b>
